...
### Changed
- **Canonical repository is now [konradvoelkel/AutoKorrektur](https://github.com/konradvoelkel/AutoKorrektur)**, a fork of `xamde/AutoKorrektur` (which was itself already a fork of the 1.0 browser version, so the fork marker costs nothing new). Build assets were re-published to a release there and `scripts/assets.manifest`, the README, the privacy policies and the website now point at it. `xamde/AutoKorrektur` is no longer maintained.
